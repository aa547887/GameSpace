...
## 文件概述
本文件基於以下權威資料來源彙整而成：
- contributing_agent.yaml - 專案開發規範與流程
- old_0905.txt - 原始業務規格書 (90% 業務邏輯)
- 
ew_0905.txt - 更新業務規格書 (90% 業務邏輯)
- database.json - 資料庫結構權威來源 (10% 技術細節)
- seedMiniGameArea.json - 展示用假資料基準
- MiniGame_area功能彙整.txt - 功能需求明細
- script_manager.sql - 管理員權限系統 SQL 腳本
- index.txt - 前端 UI 設計規範
...
#### 3.3.1 優惠券序號格式
根據 seedMiniGameArea.json 分析，商城優惠券序號格式為：
`
CPN-{年月}-{6位隨機碼}
`
...
#### 3.4.2 異動類型分析
根據 seedMiniGameArea.json 分析，異動類型包括：
...
### 6.1 權限角色定義
根據 script_manager.sql 中的 ManagerRolePermission 表定義：
