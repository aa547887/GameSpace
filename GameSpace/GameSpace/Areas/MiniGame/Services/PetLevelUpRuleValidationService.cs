    ...
    public class PetLevelUpRuleValidationService : IPetLevelUpRuleValidationService
    {
        ...
        public async Task<ValidationResult> ValidateLevelUpRuleConsistencyAsync()
        {
            ...
            try
            {
                ...
                for (int i = 1; i < rules.Count; i++)
                {
                    if (rules[i].ExperienceRequired <= rules[i-1].ExperienceRequired)
                    {
                        result.Errors.Add($"等級 {rules[i].Level} 的經驗值 ({rules[i].ExperienceRequired}) 應該大於等級 {rules[i-1].Level} 的經驗值 ({rules[i-1].ExperienceRequired})");
                        result.IsValid = false;
                    }
                }
                ...
                {
                    if (rule.PointsReward > rule.ExperienceRequired)
                    {
                        result.Warnings.Add($"等級 {rule.Level} 的點數獎勵 ({rule.PointsReward}) 大於所需經驗值 ({rule.ExperienceRequired})");
                    }

                    if (rule.ExpReward > rule.ExperienceRequired)
                    {
                        result.Warnings.Add($"等級 {rule.Level} 的經驗獎勵 ({rule.ExpReward}) 大於所需經驗值 ({rule.ExperienceRequired})");
                    }
                }
        ...
        public async Task<ValidationResult> ValidateLevelUpRuleLogicAsync(int level, int experienceRequired, int pointsReward, int expReward)
        {
            ...
            try
            {
                ...
                if (existingRules.Any())
                {
                    ...
                    if (lowerRules.Any())
                    {
                        var maxLowerExp = lowerRules.First().ExperienceRequired;
                    ...
                    if (higherRules.Any())
                    {
                        var minHigherExp = higherRules.First().ExperienceRequired;
