...
namespace GameSpace.Areas.OnlineStore.Controllers
{
    ...
    public class ProductInfoesController : Controller
    {
        ...
        )
        {
            ...
            // 投影（把寬版會用到的欄位一次備齊，避免 dynamic 找不到屬性）
            var list = await q.Select(p => new
            {
                ...
                // 供應商（Game / Other 擇一）
                SupplierName =
                    _context.GameProductDetails.Where(g => g.ProductId == p.ProductId)
                        .Select(g => _context.Suppliers.Where(s => s.SupplierId == g.SupplierId).Select(s => s.SupplierName).FirstOrDefault())
                        .FirstOrDefault()
                    ?? _context.OtherProductDetails.Where(o => o.ProductId == p.ProductId)
                        .Select(o => _context.Suppliers.Where(s => s.SupplierId == o.SupplierId).Select(s => s.SupplierName).FirstOrDefault())
                        .FirstOrDefault(),

                // 平台（只有 game 才會有）
                PlatformName = _context.GameProductDetails
                    .Where(g => g.ProductId == p.ProductId)
                    .Select(g => g.PlatformName)
                    .FirstOrDefault(),

                // 遊戲種類（game）
                GameType = _context.GameProductDetails
                    .Where(g => g.ProductId == p.ProductId)
                    .Select(g => g.GameType)
                    .FirstOrDefault(),

                // 周邊分類（nogame）
                MerchTypeName = _context.OtherProductDetails
                    .Where(o => o.ProductId == p.ProductId)
                    .Select(o => _context.MerchTypes.Where(m => m.MerchTypeId == o.MerchTypeId).Select(m => m.MerchTypeName).FirstOrDefault())
                    .FirstOrDefault()
            }).ToListAsync();
        ...
        public async Task<IActionResult> Create(ProductInfoFormVM vm)
        {
            ...
            try
            {
                ...
                // Game 的平台必選，並自動補 PlatformName（若沒填）
                if (vm.ProductType == "game")
                {
                    if (!vm.PlatformId.HasValue) return Json(new { ok = false, msg = "請選擇平台（PlatformId）。" });
                    if (string.IsNullOrWhiteSpace(vm.PlatformName))
                        vm.PlatformName = await LookupPlatformNameById(vm.PlatformId.Value);
                }
                else if (vm.ProductType == "nogame")
                {
                    if (!vm.MerchTypeId.HasValue) return Json(new { ok = false, msg = "非遊戲類需選擇周邊分類。" });
                }
                ...
                // 圖片（ImgBB URL 多筆）
                await SaveNewImageUrlsAsync(entity.ProductId, entity.ProductName, entity.ProductCreatedBy, vm.NewImageUrls);
        ...
        public async Task<IActionResult> Edit(ProductInfoFormVM vm)
        {
            ...
            // 新增 ImgBB 連結（多筆）
            await SaveNewImageUrlsAsync(p.ProductId, p.ProductName, p.ProductUpdatedBy, vm.NewImageUrls);
